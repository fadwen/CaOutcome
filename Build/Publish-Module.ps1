...
.DESCRIPTION
    ...
    working tree - Tests and their recorded What If fixtures, docs, Build, .github, and every
    object under .git.
    ...
    Examples IS shipped: the README points an installed user at the sample matrix and the
    Maester test template in it, and neither is reachable from the Gallery otherwise.

    Before staging, Build-Help.ps1 validates the help Markdown under docs/ and rebuilds
    en-US/CaOutcome-Help.xml, which is the help every exported command serves.
...
# a Private helper, say, or the en-US folder holding the MAML - it should fail here in a
# fresh process, not after someone installs it. Every command must serve the compiled help:
# with .EXTERNALHELP on each export there is no comment block to fall back on, so a missing
# or misnamed CaOutcome-Help.xml shows up as a command with no description.
#
# The fold itself is exercised too, against the recorded lockout the unit tests use, because
# a module that imports and serves help can still be missing the helper that does the work -
# and that failure only appears when a command runs. The fixture is read from the repository,
# not the staged tree: fixtures do not ship.
$stagedManifest = Join-Path $stageModule "$moduleName.psd1"
$expectedCount = @($manifest.ExportedFunctions.Keys).Count
$fixture = Join-Path $ModuleRoot 'Tests' 'Fixtures' 'promotion-locks-out-noncompliant-device.json'
...
`$recorded = Get-Content -LiteralPath '$fixture' -Raw | ConvertFrom-Json
`$outcome = ConvertTo-CaOutcome -WhatIfResult `$recorded.whatIf -SignInCondition `$recorded.signInConditions
if (-not `$outcome.Delta.BecomesEffectivelyBlocked) {
    throw 'Staged module did not find the lockout in the recorded promotion fixture.'
}
...
Write-Information 'Staged module imports, exports the declared commands, folds a recorded response, and serves MAML help for every command.' -InformationAction Continue
